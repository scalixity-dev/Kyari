...
const MetricCard: React.FC<MetricCardProps> = ({ 
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-xl shadow-md border border-white/20 hover:shadow-lg transition-shadow">
      ...
      <h3 className="text-xs sm:text-sm font-medium text-gray-800 mb-1">{title}</h3>
      {description && (
...
export default function Dashboard() {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 bg-[var(--color-happyplant-bg)] min-h-[calc(100vh-4rem)] font-sans w-full overflow-x-hidden">
      ...
      {/* Quick Actions */}
      <div className="mb-6 sm:mb-8">
        <h2 className="text-lg sm:text-xl font-semibold text-[var(--color-heading)] mb-3 sm:mb-4">Quick Actions</h2>
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
          <Link
            to="/operations/received-orders"
            className="bg-[var(--color-accent)] text-white px-4 sm:px-6 py-3 rounded-lg font-medium hover:bg-[var(--color-accent)]/90 transition-colors flex items-center justify-center gap-2 min-h-[44px]"
          >
            <Package size={18} />
            <span className="whitespace-nowrap">Verify Orders</span>
            <ChevronRight size={16} />
          </Link>
          <Link
            to="/operations/tickets"
            className="bg-blue-600 text-white px-4 sm:px-6 py-3 rounded-lg font-medium hover:bg-blue-700 transition-colors flex items-center justify-center gap-2 min-h-[44px]"
          >
            <AlertTriangle size={18} />
            <span className="whitespace-nowrap">Manage Tickets</span>
            <ChevronRight size={16} />
          </Link>
          <Link
            to="/operations/reports"
            className="bg-green-600 text-white px-4 sm:px-6 py-3 rounded-lg font-medium hover:bg-green-700 transition-colors flex items-center justify-center gap-2 min-h-[44px]"
          >
            <FileText size={18} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          <div className="bg-white rounded-xl shadow-md border border-white/20 p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 gap-3">
              ...
              <button
                ...
                className="text-sm text-[var(--color-accent)] hover:underline self-start sm:self-auto"
              >
                Mark all as read
              </button>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-2 sm:gap-3">
                      <Icon className={`w-4 h-4 sm:w-5 sm:h-5 mt-0.5 flex-shrink-0 ${colorClass}`} />
        ...
        <div className="space-y-4 sm:space-y-6">
          {/* Today's Summary */}
          <div className="bg-white rounded-xl shadow-md border border-white/20 p-4 sm:p-6">
            <h3 className="text-base sm:text-lg font-semibold text-[var(--color-heading)] mb-3 sm:mb-4">
              Today's Summary
            </h3>
